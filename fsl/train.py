...
# mode : [general, clean, addnoisy, subnoisy]
# the only difference is the training set:
# general: 600 full images per class
# clean: k1 clean images per class
# addnoisy: k1 + k2 clean & noisy images per class
# subnoisy: get rid of wrong images from [addnoisy]
...
class ModelTrainer(object):
    def __init__(self,
        ...
        #self.optimizer = optim.SGD(params=self.module_params, lr=tt.arg.lr, nesterov=True, weight_decay=1e-6, momentum=0.9)

        # set loss
        self.edge_loss = nn.BCELoss(reduction='elementwise_mean')
    ...
    def train(self):
        ...
        for iter in range(self.global_step + 1, tt.arg.train_iteration + 1):
            ...
            if tt.arg.train_transductive:
                # full_data: <batch_size, num_ways * (num_supports + num_queries), featdim>
                # edge_feat: <batch_size x num_samples x num_samples>
                full_logit_layers = self.gnn_module(full_data, init_edge)
            else:
    ...
    def finetuning(self):
        ...
        for iter in range(1,1500):
            ...
            [support_data,
             support_label,
             query_data,
             query_label] = self.data_loader['ft'].get_task_batch(num_tasks=tt.arg.meta_batch_size,
                                                                     num_ways=tt.arg.num_ways_train,
                                                                     num_shots=tt.arg.num_shots_train,
                                                                    num_queries=self.train_Query_num,
                                                                     seed=iter + tt.arg.seed)
            ...
            if self.global_step % 10 == 0:
                val_acc = self.eval(partition='test')
        ...
        with torch.no_grad():
            ...
            for iter in range(tt.arg.test_iteration//tt.arg.test_batch_size):
                ...
                if tt.arg.test_transductive:
                    full_logit_all = self.gnn_module(full_data, init_edge)
    ...
    def adjust_learning_rate(self, optimizers, lr, iter):
        ...
        #decay = 1.0 if iter < 12000 else (0.2 if iter < 18000 else (0.04 if iter < 30000 else 0.008))
        #new_lr = lr * decay
    ...
    def label2edge(self, label): # <num_tasks, num_samples>
        ...
        # expand
        #edge = edge.unsqueeze(1)   # <num_tasks, 1, num_samples, num_samples>
        #edge = torch.cat([edge, 1 - edge], 1)  # <num_tasks, 2, num_samples, num_samples>
...
if __name__ == '__main__':
    ...
    tt.arg.test_query_num = 1 if tt.arg.dataset == 'mini' else 15
    ...
    tt.arg.exp_mode = 'general' if tt.arg.exp_mode is None else tt.arg.exp_mode
    tt.arg.k1_clean = 50 if tt.arg.k1_clean is None else tt.arg.k1_clean
    tt.arg.k2_add = 600 if tt.arg.k2_add is None else tt.arg.k2_add
    ...
    tt.arg.test_iteration = 10000 if tt.arg.dataset == 'mini' else 600
    tt.arg.test_interval = 3000 if tt.arg.test_interval is None else tt.arg.test_interval
    tt.arg.test_batch_size = 100 if tt.arg.dataset == 'mini' else 30
    ...
    tt.arg.eps = 0.1
    tt.arg.dec_lr = 10000
    print('-------------------smothing:{}--------------------'.format(tt.arg.eps))
    if tt.arg.dataset == 'cub' and tt.arg.exp_mode =='general':
        tt.arg.train_iteration = 15000
        tt.arg.dec_lr = 5000

    if tt.arg.exp_mode not in ['general', 'clean', 'addnoisy', 'subnoisy']:
        print('########################## Experiment setting error !#########################')
    if tt.arg.exp_mode == 'clean':
        tt.arg.dec_lr = 5000
        print('Decay rate: %d'%(tt.arg.dec_lr))
        print('Adam initialized lr:{}, weight decay:{}'.format(tt.arg.lr, tt.arg.weight_decay))
    elif tt.arg.exp_mode == 'addnoisy' or tt.arg.exp_mode == 'subnoisy':
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        print('Experiment mode '+ tt.arg.exp_mode)
        print('k1_clean = {},  k2_add = {}'.format(tt.arg.k1_clean, tt.arg.k2_add))
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
    ...
    gnn_module = GNN(d_in = enc_module.emb_size, n_layers = tt.arg.num_layers)

    

    if tt.arg.dataset == 'mini':
        train_loader = MiniImagenetLoader(root=tt.arg.dataset_root, partition='train', mode=tt.arg.exp_mode, k1_clean=tt.arg.k1_clean, k2_add=tt.arg.k2_add)
        valid_loader = MiniImagenetLoader(root=tt.arg.dataset_root, partition='val')
    elif tt.arg.dataset == 'cub':
        train_loader = CUBLoader(root=tt.arg.dataset_root, partition='train', mode=tt.arg.exp_mode, k1_clean=tt.arg.k1_clean, k2_add=tt.arg.k2_add)
    ...
    '''
    checkpoint = torch.load('asset/checkpoints/{}/'.format(tt.arg.experiment) + 'checkpoint.pth.tar')
    trainer.enc_module.load_state_dict(checkpoint['enc_module_state_dict'])
    trainer.gnn_module.load_state_dict(checkpoint['gnn_module_state_dict'])
    trainer.global_step = checkpoint['iteration']
    tt.arg.lr = tt.arg.lr * (0.5 ** (int(checkpoint['iteration'] / tt.arg.dec_lr)))
    trainer.val_acc = checkpoint['val_acc']

    print(tt.arg.lr, trainer.global_step)
    '''
